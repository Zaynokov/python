...
class OfficeEquip:
    def __init__(self, name, quantity, color, price, service_life, paper_size, speed):
        self.color = color
        self.name = name
        self.quantity = quantity if str(quantity).isdigit() else "Unknown"
        self.price = price if str(price).isdigit() else "Unknown"
        self.service_life = service_life if str(service_life).isdigit() else "Unknown"
